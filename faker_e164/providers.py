...
class E164Provider(BaseProvider):
    ...
    def _e164(self, region_code: str, is_valid=True, is_possible=True) -> PhoneNumber:
        ...
        """
        assert not (
            is_valid and not is_possible
        ), "is_valid must be False if is_possible is False"
        e164_numerify_pattern = self._get_e164_numerify_pattern(
            region_code, is_possible=is_possible
        )
    ...
    def e164(self, region_code: str = None, valid=True, possible=True) -> str:
        ...
        return phonenumbers.format_number(
            self._e164(region_code, is_valid=valid, is_possible=possible),
    ...
    def safe_e164(self, region_code: str = None) -> str:
        ...
        phone_number = phonenumbers.parse(
            self.random_element(safe_numbers[region_code])
        )
        return phonenumbers.format_number(
    ...
    # Deprecated methods
    def _e164_phone_number(
        self, region_code, is_valid=True, is_possible=True
    ):  # pragma: no cover
        """
        Generate an e164 phone number
    ...
    def invalid_e164_phone_number(self, country=None):  # pragma: no cover
        ...
        """
        logger.warning(
            "invalid_e164_phone_number(...) will be deprecated in favor of the .e164(...) method."
        )
        countries = ["AU", "US", "GB", "NZ"]
        country = country or self.random_element(countries)
        phone_number = self._e164_phone_number(
            country,
            is_valid=False,
        )
        return phonenumbers.format_number(
    ...
    def valid_e164_phone_number(self, country=None):  # pragma: no cover
        ...
        """
        logger.warning(
            "valid_e164_phone_number(...) will be deprecated in favor of the .e164(...) method."
        )
        countries = ["AU", "US", "GB", "NZ"]
        country = country or self.random_element(countries)
        phone_number = self._e164_phone_number(
